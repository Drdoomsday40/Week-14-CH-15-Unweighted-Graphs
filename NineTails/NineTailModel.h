...
public:
	...
	void printNode(const vector<char>& node) const;
...
int NineTailModel::getFlippedNode(vector<char>& node, int position) const
{
	int row = position / 3;
...
void NineTailModel::printNode(const vector<char>& node) const
{
	for (int i = 0; i < 9; i++)
		if (i % 3 != 2)
			cout << node[i];
